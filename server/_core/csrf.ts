...
function isOriginAllowed(origin: string, host: string | string[] | undefined) {
...
export async function doubleSubmitCsrfProtection(
  ...
  try {
    // In development, skip CSRF to simplify local testing
    if (process.env.NODE_ENV !== "production") {
      return next();
    }

    // Skip auth endpoints to avoid blocking signup/login flows
