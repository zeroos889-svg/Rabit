...
for (const pkg of packagesData) {
  // حذف السجل بنفس الاسم لتجنب التكرار في غياب قيد فريد
  await pool.query(
    `DELETE FROM "consultingPackages" WHERE "name" = $1`,
    [pkg.name]
  );

  await pool.query(
    `
      INSERT INTO "consultingPackages"
        ("name", "nameEn", "description", "descriptionEn", "duration", "slaHours", "priceSAR", "features", "isActive", "orderIndex")
