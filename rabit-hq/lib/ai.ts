...
export async function generateInsights(metrics: BurnRunway): Promise<InsightResponse> {
  ...
  if (!config.featureEnableAI || !provider) {
    return {
      ...heuristic,
      enabled: false,
      source: 'heuristic',
  ...
  try {
    const res = await fetch(buildUrl(provider), {
      ...
      headers: {
        ...
        Authorization: `Bearer ${provider === 'deepseek' ? config.deepseekKey : config.openAiKey}`,
      },
      body: JSON.stringify({
        model: provider === 'deepseek' ? config.deepseekModel : 'gpt-4o-mini',
...
function buildHeuristicInsights(metrics: BurnRunway): InsightResponse {
  ...
  // Rule-based fallbacks keep the dashboard useful when AI is disabled or unreachable.
...
function buildUrl(provider: Provider) {
  if (provider === 'deepseek') {
    const base = (config.deepseekApiUrl || 'https://api.deepseek.com').replace(/\/+$/, '')
