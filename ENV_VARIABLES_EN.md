...
| -------------- | ---------------------------------- | -------------------------------------------- |
| `DATABASE_URL` | PostgreSQL database connection URL | `postgresql://user:pass@host:5432/database` |

**Database Options:**

1. **Railway PostgreSQL**: Get the URL from Railway Dashboard
2. **Local PostgreSQL**: `postgresql://postgres:password@localhost:5432/rabithr_dev`
...
| Variable          | Description                  | Example                                         |
| ----------------- | ---------------------------- | ----------------------------------------------- |
| `CLOUDINARY_URL`  | Cloudinary connection string | `cloudinary://<api_key>:<api_secret>@<cloud>`  |

**Tip:** keep the real value only in your local `.env` and avoid committing it.
...
Optional overrides (fallback defaults are used if omitted):
